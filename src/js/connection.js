...
function spawnWallet(filePath, password, onError, onSuccess) {
    ...
    if (settings.get('usePublicNode')) {
        walletdChild = spawn(getWalletdPath(), ['-w', file, '-p', password, '--server-root', directory, '--rpc-password', rpcPassword, '--daemon-address', settings.get('nodeAddress'),
     '--daemon-port', settings.get('nodePort')])
    } else {
        walletdChild = spawn(getWalletdPath(), ['-w', file, '-p', password, '--server-root', directory, '--rpc-password', rpcPassword]) 
    }
...
function createWallet (dir, name, password, onError, onSuccess) {
    ...
    let child = execFile(getWalletdPath(), ['-w', filename, '-p', password, '--server-root', dir, '-g', '--rpc-password', rpcPassword],
     (error, stdout, stderr) => {
        if (error) {
...
function importWallet (dir, name, password, viewKey, spendKey, onError, onSuccess) {
    ...
    let child = execFile(getWalletdPath(), ['-w', filename, '-p', password, '--server-root', dir, '-g', '--view-key', viewKey, '--spend-key', spendKey, '--rpc-password', rpcPassword], 
     (error, stdout, stderr) => {
        if (error) {
